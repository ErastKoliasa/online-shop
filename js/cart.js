class Cart {
  ...
  async renderCart() {
    ...
    for (const productId in this.cart) {
      ...
      cartDomString += this.createCartProductDomString(product, rate);
      total += (product.price * rate).toFixed(2) * this.cart[productId];
    }
  ...
  createCartProductDomString(product, rate) {
    return `<div class="row" data-id="${product.id}"> 
              <div class="col-5">${product.title}</div>
              <div class="col-3">${(product.price * rate).toFixed(2)}</div>
  ...
  async order(ev) {
    ...
    fetch("https://formspree.io/f/mdovzwel", {
      method: "POST",
      headers: {
